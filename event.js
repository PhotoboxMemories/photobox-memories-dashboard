...
(function (G) {
  ...
  #ws .wtop{height:58px;background:#221b45;color:#fff;display:flex;align-items:center;gap:14px;padding:0 18px;flex:none}
  #ws .wtop .logo{font-weight:800;font-size:19px}#ws .wtop .logo span{color:#ff5c93}
  ...
  #ws nav.side a:hover{background:#f7f3f8}
  #ws nav.side a.on{background:#f5eef4;border-left-color:#e83f78;font-weight:600}
  ...
  #ws .card{background:#fff;border-radius:14px;padding:18px 20px;box-shadow:0 1px 3px rgba(0,0,0,.06);max-width:620px;margin-bottom:14px}
  ...
  #ws .tabs button.on{color:#e83f78;border-color:#e83f78;font-weight:600}
  ...
  #ws .sw input:checked+i{background:#e83f78}#ws .sw input:checked+i:after{left:21px}
  ...
  #ws .slider input[type=range]{accent-color:#e83f78;width:100%}
  #ws .btn{border:1px solid #e83f78;color:#e83f78;background:#fff;border-radius:999px;padding:8px 18px;cursor:pointer;font:inherit;font-weight:600}
  #ws .btn.pri{background:#e83f78;color:#fff}
  ...
  #ws .lay:hover{border-color:#e83f78}
  ...
  #ws .dd .menu button:hover{background:#f5eef4}
  ...
  #ws .mode.on{background:#b0305f;color:#fff}
  ...
  #ws .soon{display:inline-block;font-size:11px;background:#fde8f0;color:#c2185b;border-radius:999px;padding:2px 8px;margin-left:6px;font-weight:600}
  ...
  #wsModal .cats button.on{background:#e83f78;border-color:#e83f78;color:#fff}
  ...
  #wsModal .tp:hover img{border-color:#e83f78}
  ...
  #wsModal .fi.on{border-color:#e83f78}
  ...
  #wsModal .btn{border:1px solid #e83f78;background:#e83f78;color:#fff;border-radius:999px;padding:9px 20px;cursor:pointer;font:inherit;font-weight:600}
  `;
  ...
  const DEF = {
    title: '', subtitle: '', startText: '📸 Tippen zum Starten', accent: '#ff3d7f', font: 'modern', frame: '#ffffff', textColor: '#222222', template: 'classic',
  ...
  const NAV = [
    ['dash', '🖥', 'Dashboard'],
    ['h', 'Grundlagen'], ['layout', '◎', 'Photo booth experience'], ['capture', '☰', 'Aufnahmeeinstellungen'], ['voice', '🗣', 'Virtueller Berater'],
    ['h', 'Erweiterte Einstellungen'], ['start', '▢', 'Startbildschirm'], ['fx', '✦', 'Filter & Effekte'], ['bg', '▨', 'Hintergrundentfernung'],
    ['ai', '✧', 'KI-Porträts'], ['survey', '☷', 'Umfrage & Haftungsausschluss'],
    ['h', 'Drucken & Teilen'], ['share', '⇪', 'Einstellungen für Teilen'], ['print', '⎙', 'Druckeinstellungen'], ['gallery', '▦', 'Online-Galerie']
  ];
  ...
  function open(ev, ctx) {
    ...
    root.innerHTML = `<div class="wtop"><div class="logo">📸 Photobox <span>Memories</span></div><span class="en"></span><span class="sp"></span>
      <span class="st"></span><button class="tb" data-a="preview">👁 Vorschau</button></div>
    ...
    function renderNav() {
      side.innerHTML = `<span class="back" data-a="back">‹ Zurück zu den Ereignissen</span>` + NAV.map(n => n[0] === 'h'
        ? `<h6>${n[1]}</h6>` : `<a data-p="${n[0]}" class="${n[0] === page ? 'on' : ''}"><span>${n[1]}</span>${n[2]}</a>`).join('');
      $(root, '.en').textContent = E.name;
    }
    side.addEventListener('click', e => {
      const a = e.target.closest('a[data-p]'); if (a) { page = a.dataset.p; renderNav(); render(); }
      if (e.target.dataset.a === 'back') close();
    });
    $(root, '.wtop').addEventListener('click', e => { if (e.target.dataset.a === 'preview') previewAll(); });
    ...
    function layoutCards() {
      return `<div class="dd"><button class="btn" data-a="newlay">+ Neues Layout ⌄</button><div class="menu" style="display:none">
          <button data-a="fromtpl">▦ Vorlage wählen</button><button data-a="fromimg">⤒ Bild hochladen</button></div></div>
        <div class="lays">${S.layouts.length ? S.layouts.map(L => `<div class="lay" data-a="editlay" data-id="${L.id}">
            <div class="acts"><button data-a="duplay" data-id="${L.id}" title="Duplizieren">⧉</button><button data-a="dellay" data-id="${L.id}" title="Löschen">🗑</button></div>
    ...
    const PAGES = {
      dash() {
        ...
        return `<h1>Dashboard</h1>
          ...
          <button class="ghost" data-a="delev" style="color:#d23b3b">🗑 Ereignis löschen</button>`;
      },
      layout() {
      ...
      capture() {
        sub.mode ||= 'photo';
        const M = [['photo', '📷', 'Foto', 'single'], ['gif', '🎞', 'GIF', 'gif'], ['boom', '🔁', 'Boomerang', 'boomerang'], ['video', '🎥', 'Video', 'video']];
        ...
        return `<h1>Aufnahmeeinstellungen</h1><div class="cols"><div class="left">
          <div class="modes">${M.map(([k, ic, n, key]) => `<div class="mode ${sub.mode === k ? 'on' : ''}" data-mode="${k}"><span class="ic">${ic}</span>${n}<span class="dot ${(k === 'photo' ? photoOn : S.modes[key]) ? 'a' : ''}"></span></div>`).join('')}</div>
      ...
      start() {
        return `<h1>Startbildschirm</h1><div class="cols"><div class="left"><div class="card">
          ...
          <label class="l">Logo</label><div class="row">${E.logo_url ? `<img src="${esc(E.logo_url)}" style="height:50px;border-radius:6px"><button class="ghost" data-a="dellogo">Entfernen</button>` : ''}<button class="ghost" data-a="uplogo">⤒ Logo hochladen</button></div>
          <div class="row" style="margin-top:12px"><div><label class="l">Akzentfarbe</label><input type="color" data-s="accent" value="${S.accent}"></div></div>
          <p style="color:#888;font-size:12px">Hintergrundbild oder -video für den Startbildschirm wird direkt in der FotoBox hinterlegt (Admin → Design).</p></div></div>${startPreview()}</div>`;
      },
      fx() {
      ...
      bg() {
        return `${titleSw('Hintergrundentfernung', '')}<div class="card"><b>Ersetzen Sie den Hintergrund Ihrer Fotos</b>
          ...
          <button class="ghost" data-a="upbg" style="margin-top:10px">⤒ Bild hochladen (JPG/PNG, empfohlen 1800×1200)</button></div>`;
      },
    ...
    function previewBox() {
      return `<div class="pv"><div class="frame"><canvas id="wsPv"></canvas><div class="row" style="justify-content:space-between;margin-top:12px">
        <button class="btn pri" data-a="edit1">✎ Bearbeiten</button><small style="color:#777">${S.layouts[0] ? esc(S.layouts[0].name) : 'Standard-Vorlage'}</small></div></div></div>`;
    }
    function startPreview() {
      return `<div class="pv"><div class="frame"><div id="wsStart" style="aspect-ratio:3/4;border-radius:10px;background:linear-gradient(160deg,#556,#99a);display:flex;flex-direction:column;align-items:center;justify-content:center;color:#fff;text-align:center;padding:20px">
        ...
        <div style="opacity:.85">${esc(S.subtitle)}</div><div style="margin-top:30px;background:${S.accent};padding:10px 22px;border-radius:999px;font-weight:700">${esc(S.startText)}</div></div></div></div>`;
    }
    const fThumbCache = {};
